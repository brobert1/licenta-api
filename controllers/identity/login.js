...
export default async (req, res) => {
  ...
  const { id, name, active, __t: role, image, password: passwordFromDb } = identity;
  // Accounts do not have the "confirmed" key as they are not required to confirm their email
  if (!active) {
  ...
  const payload = { name, email, role, me: id, image };
